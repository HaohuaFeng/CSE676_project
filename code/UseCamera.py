...
def camera(width=1920, high=1080):
    ...
    font_scale = 0.5
    font_color = (0, 0, 255)  # BGR color (green in this case)
    thickness = 1
    ...
    face_reg = cv2.CascadeClassifier(cv2.data.haarcascades + 'haarcascade_frontalface_default.xml')
    ...
    with torch.no_grad():
        while capture.isOpened():
            ...
            face_image = None
            faces = face_reg.detectMultiScale(gray, scaleFactor=1.25, minNeighbors=5, minSize=(48, 48))
            ...
            if face_image is not None:
                ...
                y = org[1]
                for line in lines:
                    frame = cv2.putText(frame, line, (org[0], y), font, font_scale, font_color,
